...
BOOST_AUTO_TEST_CASE(test_parser) {
        std::string srcdir(std::getenv("SRCDIR") ? std::getenv("SRCDIR") : ".");
        BindParser BP;
        BOOST_CHECK_THROW( BP.parse(srcdir + "/../regression-tests/named.confx"), PDNSException);
        BP.setVerbose(true);

        BP.parse(srcdir + "/auth/named.conf.parsertest");
